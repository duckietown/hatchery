...
class RosVersionDetailDialog : DialogWrapper(null, true) {
  ...
  override fun createCenterPanel(): JComponent {
    ...
    mainPanel = panel
    versionList.cellRenderer =
      object : ColoredListCellRenderer<RosVersionImpl>() {
  ...
  private fun removeSdk() =
    versionList.selectedValue?.let {
      ...
      if (customVersion.contains(it)) customVersion.remove(it)
      val state = getRosEnvironment()
      if (state.isDefaultVersion(it.name)) customVersion.removeDefault(it.name)
      state.remove(it)
      refreshVersionList()
